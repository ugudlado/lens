...
describe('scanHooks', () => {
    ...
    it('reads disableAllHooks flag from project settings', async () => {
        await writeFile(join(projectDir, '.claude', 'settings.json'), JSON.stringify({ disableAllHooks: true }));
        const scanHooks = await getScanHooks();
        const result = await scanHooks(projectDir);
    ...
    it('parses hooks from project settings.json', async () => {
        ...
        await writeFile(join(projectDir, '.claude', 'settings.json'), JSON.stringify(config));
        const scanHooks = await getScanHooks();
        const result = await scanHooks(projectDir);
    ...
    it('substitutes ${CLAUDE_PLUGIN_ROOT} in plugin hook commands', async () => {
        ...
        await writeFile(join(pluginInstallPath, 'hooks', 'hooks.json'), JSON.stringify(hooksJson));
        const scanHooks = await getScanHooks();
        const result = await scanHooks(projectDir, [
