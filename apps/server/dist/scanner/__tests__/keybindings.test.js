...
describe('scanKeybindings', () => {
    ...
    it('reads keybinding entries with key, command, and when fields', async () => {
        ...
        await writeFile(keybindingsPath, JSON.stringify([
            { key: 'ctrl+shift+p', command: 'workbench.action.showCommands', when: 'editorFocus' },
            { key: 'ctrl+s', command: 'workbench.action.files.save', when: 'editorIsOpen' },
        ]));
        const scanKeybindings = await getScanKeybindings();
        const result = await scanKeybindings();
    ...
    it('entry missing when field has when as undefined', async () => {
        const keybindingsPath = join(globalDir, 'keybindings.json');
        await writeFile(keybindingsPath, JSON.stringify([{ key: 'ctrl+k', command: 'editor.action.trimTrailingWhitespace' }]));
        const scanKeybindings = await getScanKeybindings();
        const result = await scanKeybindings();
        expect(result.entries).toHaveLength(1);
        expect(result.entries[0].key).toBe('ctrl+k');
        expect(result.entries[0].command).toBe('editor.action.trimTrailingWhitespace');
