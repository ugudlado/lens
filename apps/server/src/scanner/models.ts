...
export async function scanModels(
  effectiveSettings: Record<string, ScopedItem<unknown>>,
): Promise<ModelsSurface> {
  ...
  providers.push({
    ...
    available:
      !!process.env.ANTHROPIC_BEDROCK_BASE_URL || !!process.env.AWS_REGION,
    models: [],
  ...
  const defaultModel = effectiveSettings.model
    ? (effectiveSettings.model as ScopedItem<string>)
    : null;
