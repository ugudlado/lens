...
describe("scanPlugins", () => {
  ...
  it("parses v2 format installed_plugins.json", async () => {
    ...
    await writeFile(
      join(globalDir, "plugins", "installed_plugins.json"),
  ...
  it("suppresses orphaned plugin entry when same plugin exists from known marketplace", async () => {
    ...
    await writeFile(
      join(globalDir, "plugins", "known_marketplaces.json"),
      JSON.stringify({
        "known-marketplace": {
          repo: "https://github.com/example/known",
          installLocation: knownInstallPath,
        },
      }),
    );
  ...
  it("applies enabledPlugins: project settings override global settings", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        enabledPlugins: { "toggled-plugin@test-marketplace": false },
      }),
    );

    // Verify global disables the plugin
    vi.resetModules();
    const scanPlugins1 = (await import("../plugins.js")).scanPlugins;
    ...
    await writeFile(
      ...
      JSON.stringify({
        enabledPlugins: { "toggled-plugin@test-marketplace": true },
      }),
    );
  ...
  it("scans marketplace plugins/ directory and marks installed status", async () => {
    ...
    await writeFile(
      join(pluginADir, "README.md"),
      "The description for plugin-a.",
    );
    await writeFile(
      join(pluginBDir, "README.md"),
      "The description for plugin-b.",
    );
      ...
      JSON.stringify({
        ...
        plugins: {
          "plugin-a@test-marketplace": [
            {
              installPath: pluginAInstallPath,
              version: "1.0.0",
              installedAt: "",
              scope: "user",
            },
    ...
    expect(result.marketplaces[0].url).toBe(
  ...
  it("sets updateAvailable correctly based on version comparison", async () => {
    ...
    const semverPluginMarketplaceDir = join(
      marketplaceDir,
      "plugins",
      "semver-plugin",
    );
    await mkdir(semverPluginMarketplaceDir, { recursive: true });
    await mkdir(join(semverPluginMarketplaceDir, ".claude-plugin"), {
      recursive: true,
    });
    await writeFile(
      join(semverPluginMarketplaceDir, ".claude-plugin", "plugin.json"),
    ...
    await writeFile(
      join(upToDatePluginDir, ".claude-plugin", "plugin.json"),
      ...
      JSON.stringify({
        ...
        plugins: {
          ...
          "up-to-date-plugin@versioned-marketplace": [
            {
              installPath: upToDateInstallPath,
              version: "1.0.0",
              installedAt: "",
              scope: "user",
            },
    ...
    expect(result.available).toHaveLength(2);
    const availableSemver = result.available.find(
      (p) => p.name === "semver-plugin",
    );
    const availableUpToDate = result.available.find(
      (p) => p.name === "up-to-date-plugin",
    );
