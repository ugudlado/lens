...
export function getAllowGlobalWrites(): boolean {
  return _allowGlobalWrites;
}
export function setAllowGlobalWrites(value: boolean): void {
...
export const GLOBAL_DIR =
  process.env.__TEST_GLOBAL_DIR ?? join(homedir(), ".claude");
...
export function settingsLocations(
  ...
  return [
    {
      path: join(MANAGED_DIR, "managed-settings.json"),
