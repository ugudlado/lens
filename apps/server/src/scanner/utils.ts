...
export function getAllowGlobalWrites(): boolean { return _allowGlobalWrites; }
export function setAllowGlobalWrites(value: boolean): void { _allowGlobalWrites = value; }

/** Returns whether a config item at the given scope is editable. */
export function isEditable(scope: ConfigScope): boolean {
...
export async function readJsonOrNull(path: string): Promise<Record<string, unknown> | null> {
...
export function settingsLocations(projectPath: string): { path: string; scope: ConfigScope }[] {
  return [
...
export function scopeForPath(filePath: string, projectPath: string): ConfigScope {
  const resolved = resolve(filePath);
  if (resolved.startsWith(resolve(MANAGED_DIR))) return ConfigScope.Managed;
  if (resolved.includes('.local.') || resolved.endsWith('settings.local.json')) return ConfigScope.Local;
  if (resolved.startsWith(resolve(projectPath))) return ConfigScope.Project;
  if (resolved.startsWith(resolve(GLOBAL_DIR))) return ConfigScope.Global;
