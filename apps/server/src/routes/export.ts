    ...
    if (requestedSections.includes('skills')) {
      sections.skills = await Promise.all(
        config.skills.skills
          ...
          .map(async s => {
            let content = '';
            try { content = await readFile(s.filePath, 'utf-8'); } catch { /* skip */ }
    ...
    if (requestedSections.includes('agents')) {
      sections.agents = await Promise.all(
        config.agents.agents
          ...
          .map(async a => {
            let content = '';
            try { content = await readFile(a.filePath, 'utf-8'); } catch { /* skip */ }
      ...
      sections.rules = await Promise.all(
        config.rules.rules
          ...
          .map(async r => {
            ...
            return { name: r.name, content: r.content, ext: ext as 'md' | 'mdc' };
          })
      );
    }
