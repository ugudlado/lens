...
function sectionFromHash(): NavSection {
  ...
  return NAV_SECTION_VALUES.has(hash)
    ? (hash as NavSection)
    : NavSection.Overview;
}
...
export default function App() {
  ...
  const fetchConfig = useCallback(
    (projectPath?: string) => {
      ...
      const url = project
        ? `/api/config?project=${encodeURIComponent(project)}`
        : "/api/config";
      return fetch(url)
        .then((res) => {
          if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  const fetchSuggestions = useCallback(
    (projectPath?: string) => {
      const project = projectPath ?? activeProject;
      const url = project
        ? `/api/suggestions?project=${encodeURIComponent(project)}`
        : "/api/suggestions";
      return fetch(url)
        .then((res) => {
          if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  useEffect(() => {
    if (activeProject) {
      ...
      void Promise.all([
        ...
        fetchSuggestions(activeProject),
      ]);
    }
    ...
    eventSource.addEventListener("config-changed", (e) => {
      try {
        ...
        if (!data.projectPath || data.projectPath === activeProject) {
          void fetchConfig(activeProject ?? undefined);
          void fetchSuggestions(activeProject ?? undefined);
        }
      } catch {
        void fetchConfig(activeProject ?? undefined);
        void fetchSuggestions(activeProject ?? undefined);
  ...
  const handleNavigateToResult = useCallback(
    (targetSection: NavSection, scrollId?: string) => {
  ...
  async function handleRemoveWorkspace(name: string) {
    try {
      ...
      if (
        activeProject &&
        workspaces.find((w) => w.name === name)?.path === activeProject
      ) {
