...
export function PermissionsPanel({ config, onRescan }: Props) {
  ...
  const filteredRules = rules.filter((r) => {
    ...
    if (!q) return true;
    return r.rule.toLowerCase().includes(q) || r.type.toLowerCase().includes(q);
  });
  ...
  async function addRule(type: PermissionType) {
    ...
    const existingRule = rules.find(
      (r) => r.type === type && r.scope !== ConfigScope.Managed,
    );
  ...
  function getScopeOptions(rule: PermissionRule) {
    ...
    if (
      rule.scope === ConfigScope.Project ||
      rule.scope === ConfigScope.Local
    ) {
  ...
  async function saveEdit() {
    ...
    if (trimmed === rule.rule) {
      setEditingRule(null);
      return;
    }

    const fileRules = rules.filter(
      (r) => r.type === rule.type && r.filePath === rule.filePath,
    );
    const updatedRules = fileRules.map((r, idx) =>
      idx === editingRule.index ? trimmed : r.rule,
    );
      ...
      {view === "json" ? (
        ...
        <>
          ...
          {(
            ...
            const cfg =
              TYPE_BADGE_STYLES.permission[
                type as keyof typeof TYPE_BADGE_STYLES.permission
              ] ?? TYPE_BADGE_STYLES.permission.allow;
            return (
              <div key={type} className="mb-6">
                <div className="mb-3 flex items-center gap-2">
                  <span
                    className={`rounded px-2 py-0.5 text-xs font-medium ${cfg.bg} ${cfg.text}`}
                  >
                ...
                <div className="divide-y divide-border rounded-lg border border-border bg-card">
                  {typeRules.map((rule, i) => {
                    ...
                    const fileRules = rules.filter(
                      (r) =>
                        r.type === rule.type && r.filePath === rule.filePath,
                    );
                    const fileIndex = fileRules.findIndex((r) => r === rule);
                    const isEditing =
                      editingRule !== null &&
                      editingRule.index === fileIndex &&
                      editingRule.rule.filePath === rule.filePath &&
                      editingRule.rule.type === rule.type;
                    return (
                      <div
                        key={i}
                        id={`permission-${slug(rule.rule)}-${i}-${rule.scope}`}
                        ...
                        {isEditing ? (
                          <>
                            <input
                              ...
                              autoFocus
                              disabled={saving}
                              className="flex-1 rounded border border-accent bg-bg px-3 py-1 font-mono text-sm text-gray-200 focus:outline-none disabled:opacity-50"
                            />
                            <button
                              onClick={() => void saveEdit()}
                              disabled={saving || !editingRule.newText.trim()}
                              className="rounded bg-accent/20 px-2.5 py-1 text-xs font-medium text-accent transition-colors hover:bg-accent/30 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                              Save
                            </button>
                            <button
                              ...
                              disabled={saving}
                              className="rounded bg-gray-700/40 px-2.5 py-1 text-xs font-medium text-gray-400 transition-colors hover:text-gray-200 disabled:opacity-50"
                            >
                              Cancel
                            </button>
                          </>
                          ...
                          <>
                            <code className="flex-1 truncate font-mono text-sm text-gray-200">
                              {rule.rule}
                            </code>
                            <ScopeIndicator scope={rule.scope} />
                            <button
                              onClick={() =>
                                jumpToFile(rule.rule, rule.filePath)
                              }
                              className="max-w-[200px] truncate font-mono text-xs text-gray-600 transition-colors hover:text-accent"
