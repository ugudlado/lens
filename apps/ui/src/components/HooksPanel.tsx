  ...
  for (const [event, arr] of map.entries()) {
    map.set(
      ...
      arr.sort((a, b) => {
        ...
        return sd !== 0
          ? sd
          : (a.command ?? a.prompt ?? "").localeCompare(
              b.command ?? b.prompt ?? "",
            );
      }),
    );
...
function canRemoveHook(hook: HookEntry): boolean {
  return (
    hook.scope !== ConfigScope.Managed && hook.source === HookSource.Settings
  );
}
...
export function HooksPanel({ config, onRescan }: Props) {
  ...
  const defaultFile =
    editableSettingsFiles.find((f) => f.scope === ConfigScope.Project) ??
  ...
  async function removeHook(hook: HookEntry) {
    const sameFileEventHooks = hooks.filter(
      (h) =>
        h.event === hook.event &&
        h.filePath === hook.filePath &&
        h.source === HookSource.Settings,
    );
  ...
  async function toggleDisableAll() {
    const settingsFile =
      config.settings.files.find((f) => f.scope === ConfigScope.Global) ??
      config.settings.files[0];
  ...
  function getHookScopeOptions(hook: HookEntry): {
    label: string;
    scope?: ConfigScope;
    onCopy: () => Promise<void>;
    onMove?: () => Promise<void>;
    ...
    if (hook.scope === ConfigScope.Project) {
      const globalFile = config.settings.files.find(
        (f) => f.scope === ConfigScope.Global,
      );
    ...
    if (hook.scope === ConfigScope.Global) {
      const projectFile = config.settings.files.find(
      ...
      return [
        {
          ...
          onCopy: () =>
            copyHook(hook, ConfigScope.Project, projectFile.filePath),
          onMove: config.allowGlobalWrites
            ? () => moveHook(hook, ConfigScope.Project, projectFile.filePath)
            : undefined,
        },
      ];
    }
  ...
  async function addHook() {
    ...
    const existingHooks = hooks.filter(
      (h) =>
        h.event === addForm.event &&
        h.filePath === addForm.filePath &&
        h.source === HookSource.Settings,
    );
  ...
  const jsonFiles = config.settings.files.map((f) => ({
    scope: f.scope,
    filePath: f.filePath,
  }));
  ...
  const allExpanded =
    allGroupKeys.length > 0 && allGroupKeys.every((k) => expanded.has(k));
          ...
          <div className="mb-4 flex items-center gap-3 rounded-lg border border-border bg-card px-4 py-3">
            ...
            {disableAllHooks && (
              <span className="text-xs text-red-400/60">
                All hooks are currently disabled
              </span>
            )}
          </div>
            ...
            <div className="mb-4 space-y-3 rounded-lg border border-border bg-card p-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAddForm((prev) => ({
                      ...prev,
                      event: e.target.value as HookEvent,
                    }))
                  }
                  className="w-full rounded border border-border bg-bg px-3 py-1.5 text-sm text-gray-200 focus:border-accent focus:outline-none"
                >
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={
                    addForm.type === HookType.Command
                      ? "e.g. npm run lint"
                      : "e.g. Check for security issues"
                  }
                  className="w-full rounded border border-border bg-bg px-3 py-1.5 font-mono text-sm text-gray-200 placeholder:text-gray-600 focus:border-accent focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAddForm((prev) => ({ ...prev, matcher: e.target.value }))
                  }
                ...
                >
                  {editableSettingsFiles.map((f) => {
                    const shortPath = f.filePath
                      ...
                      .replace(/.*\/([^/]+)$/, "$1");
                    const scopeLabel =
                      f.scope.charAt(0).toUpperCase() + f.scope.slice(1);
                    return (
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button
                  onClick={() => setShowAddForm(false)}
                  disabled={saving}
                  className="rounded bg-gray-500/20 px-4 py-1.5 text-xs font-medium text-gray-400 transition-colors hover:bg-gray-500/30 disabled:opacity-50"
                >
                  Cancel
                </button>
              </div>
                ...
                >
                  <div className="divide-y divide-border border-t border-border">
                    {eventHooks.map((hook, i) => {
                      const typeBadge =
                        TYPE_BADGE_STYLES.hook[
                          hook.type as keyof typeof TYPE_BADGE_STYLES.hook
                        ] ?? TYPE_BADGE_STYLES.hook.command;
                      const srcBadge =
                        SOURCE_BADGES[hook.source] || SOURCE_BADGES.settings;
                      ...
                      const isEditing =
                        editing !== null && editing.key === editKey;

                      return (
                        <div
                          key={i}
                          id={`hook-${slug(hook.event)}-${i}-${hook.scope}`}
                        ...
                        >
                          <div className="mb-2 flex flex-wrap items-center gap-2">
                            ...
                            {canRemoveHook(hook) && (
                              <div className="ml-auto flex items-center gap-1">
                                {!isEditing &&
                                  (() => {
                                    const hookScopeOptions =
                                      getHookScopeOptions(hook);
                                    return hookScopeOptions.length > 0 ? (
                                      <ScopeMoveButton
                                        options={hookScopeOptions}
                                        saving={saving}
                                      />
                                    ) : null;
                                  })()}
                          ...
                          {isEditing ? (
                            <div className="space-y-2">
                              ...
                              <input
                                ...
                                className="w-full rounded border border-border bg-bg px-2 py-1 font-mono text-sm text-gray-200 placeholder:text-gray-600 focus:border-accent focus:outline-none"
                              />
                              {/* Save / Cancel */}
                          ...
                          <button
                            onClick={() =>
                              jumpToFile(hook.event, hook.filePath)
                            }
                            className="mt-2 block truncate font-mono text-xs text-gray-600 transition-colors hover:text-accent"
