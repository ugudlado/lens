    ...
    <div>
      ...
      <div className="mb-8">
        <div className="flex items-start justify-between">
          <div>
            <h2 className="mb-2 text-2xl font-bold">Configuration Overview</h2>
          ...
          <div className="flex items-center gap-2">
            {onRescan && (
              <button
                ...
                className="flex flex-shrink-0 items-center gap-1.5 rounded bg-accent/20 px-3 py-1.5 text-xs font-medium text-accent transition-colors hover:bg-accent/30"
              >
                <span>↓</span>
                Import Config
              </button>
            ...
            <button
              ...
              className="flex-shrink-0 rounded bg-[#6c5ce7]/20 px-3 py-1.5 text-xs font-medium text-[#6c5ce7] transition-colors hover:bg-[#6c5ce7]/30"
            >
              ↑ Export Config
            </button>
          </div>
        ...
        {CARDS.map((card) => {
          ...
          return (
            <button
              ...
              className={`group rounded-lg border border-border bg-card p-5 text-left transition-all ${
                count === 0
                  ? "cursor-default opacity-50"
                  : "cursor-pointer hover:border-accent/50 hover:bg-card/80"
              }`}
            >
              <div className="text-3xl font-bold text-accent transition-colors group-hover:text-accent-hover">
                {count}
              </div>
              ...
              {scopeCounts.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-x-3 gap-y-0.5 text-xs">
                  {scopeCounts.map(({ scope, count }) => (
                    <span
                      key={scope}
                      className={SCOPE_TEXT_COLORS[scope] || "text-gray-400"}
                    >
                      {count} {scope}
                    </span>
                  ))}
