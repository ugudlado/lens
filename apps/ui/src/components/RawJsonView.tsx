...
export function RawJsonView({
  ...
  const { update, saving, error } = useConfigUpdate(() => {
    ...
    if (savedIdx !== null) {
      void loadContent(
        savedIdx,
        uniqueRef.current[savedIdx]?.filePath || files[savedIdx]?.filePath,
      );
    }
    onRescan();
  });
  ...
  const unique = files.filter(
    (f, i, arr) => arr.findIndex((x) => x.filePath === f.filePath) === i,
  );
    ...
    setExpanded((prev) => {
      ...
      if (next.has(idx)) {
        ...
        if (!fileContents[idx] && !loading.has(idx)) {
          void loadContent(idx, uniqueRef.current[idx].filePath);
        }
      }
      return next;
    });
  ...
  useEffect(() => {
    ...
    const idx = uniqueRef.current.findIndex(
      (f) => f.filePath === autoExpandFile,
    );
    if (idx === -1) return;
                  ...
                  if (isEditing) {
                    return (
                      <>
                        ...
                        />
                        {parseError && (
                          <div className="mt-2 rounded border border-red-500/30 bg-red-500/10 px-3 py-2 font-mono text-xs text-red-400">
                            {parseError}
                          </div>
                        )}
                  ...
                  return (
                    <>
                      <HighlightedPre
                        ...
                        highlightLine={isHighlighted ? highlightLine : null}
                      />
...
function HighlightedPre({
  ...
  return (
    <pre
      ref={preRef}
      className="max-h-96 overflow-x-auto overflow-y-auto whitespace-pre-wrap font-mono text-sm text-gray-300"
    >
      {lines.map((line, idx) => {
        const lineNum = idx + 1;
        ...
        return (
          <span
            ...
            className={
              isTarget ? "-mx-1 block rounded bg-accent/20 px-1" : undefined
            }
          >
            {line}
            {idx < lines.length - 1 ? "\n" : ""}
